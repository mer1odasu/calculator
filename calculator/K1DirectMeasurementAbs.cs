...
{
    [ModelDefault("Caption", "К1 Прямое измерение, абсолютная погрешность"), NavigationItem("МИ П.16-2021")]
    public class K1DirectMeasurementAbs : MI_P16Base
    {
        public K1DirectMeasurementAbs (Session session) : base(session) { }

        //[DisplayName("Единица измерений")]
        //public dUnitKind UnitKind { get; set; }

        private double i_AbsoluteError;
        [DisplayName("Абсолютная погрешность [Δ]")]
        public double AbsoluteError
        {
            get { return Math.Round(i_AbsoluteError, Capacity); }
            set {
                bool changed = SetPropertyValue(nameof(AbsoluteError), ref i_AbsoluteError, value);
        ...
        {
            UncertaintyBType = AbsoluteError / Math.Sqrt(3);
